  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className={styles.tooltip}
        style={{backgroundColor: "#E60000", color: "#FFFFFF", padding: "10px 10px", margin: "10px"}}>
    ...
    <div className={styles.wrapper}>
      <div className={styles.headerContainer}>
        ...
        <div className={styles.legendContainer}>
          {legendData.map((item) => (<div key={item.dataKey} className={styles.legendItem}>
            <span className={styles.legendDot} style={{backgroundColor: item.dataKey === "kilogram" ? "#282D30" : "#E60000"}}/>
            <span>{item.label}</span>
          </div>))}
        </div>
      </div>
      <ResponsiveContainer width="100%" height={200} backgroundColor={"#9B9EAC"}>
        <BarChart data={data} barGap={8} margin={{top: 20, right: -30, left: 0 , bottom: 0}}>
          ...
          <ReferenceLine strokeDasharray="3 3" yAxisId="kg" y={79} stroke="#DEDEDE"/>
