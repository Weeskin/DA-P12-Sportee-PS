...
export function useDashboard(userId) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [info, activityData, avgSessions, perf] = await Promise.all([getUserInfo(userId), getUserActivity(userId), getUserAverageSessions(userId), getUserPerformance(userId)]);
