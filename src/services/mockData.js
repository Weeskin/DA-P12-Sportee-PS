/**
 * Données mockées — miroir exact du backend.
 * Utilisées quand USE_MOCK = true dans api.jsx
 */
...
export const getMockUserInfo = (id) =>
  USER_MAIN_DATA.find((user) => user.id === id) ?? null;
export const getMockUserActivity = (id) =>
  USER_ACTIVITY.find((user) => user.userId === id) ?? null;
